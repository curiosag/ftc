...
public class TweakedTemplateProposal implements ICompletionProposal, ICompletionProposalExtension,
		ICompletionProposalExtension2, ICompletionProposalExtension3 {

	private final Template fTemplate;
	private final FtcDocumentTemplateContext fContext;
	...
	public void apply(ITextViewer viewer, char trigger, int stateMask, int offset) {
		...
		try {
			...
			for (int i = 0; i != variables.length; i++) {
				...
				LinkedPosition first;
				{
					...
					if (proposals.length > 1)
						first = new TweakedProposalPosition(variable, getCategory(), fContext, offsets[0] + start,
								length, proposals);
					else
						first = new LinkedPosition(document, offsets[0] + start, length);	
					
				}
				if (tablePosition == null && variable.getType().startsWith("t"))
					tablePosition = first;
			...
			if (hasPositions) {
				model.forceInstall();
				TweakedLinkedModeUI ui = new TweakedLinkedModeUI(model, viewer);
				ui.setExitPosition(viewer, getCaretOffset(templateBuffer) + start, 0, Integer.MAX_VALUE);
				
				try {
					ui.enter(tablePosition);	
	...
	private void openErrorDialog(Shell shell, Exception e) {
		MessageDialog.openError(shell, "There's an issue..", e.getMessage());
	}
