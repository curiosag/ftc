...
class TipUtil {
	 ...
	 * that it doesn't use its RGB values (uses HSB instead?) and so querying
	 * them is useless.
	 *
	 * @param c
	 *            The color to check.
	...
	/**
	 * Tweaks a <code>JEditorPane</code> so it can be used to render the content
	 * in a focusable pseudo-tool tip. It is assumed that the editor pane is
	 * using an <code>HTMLDocument</code>.
	 *
	 * @param textArea
